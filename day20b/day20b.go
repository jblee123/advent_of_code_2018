...
func findNumFarRooms(board [][]byte, pos mapCoord, minDoors int) int {
	cols := len(board)
	rows := len(board[0])
	...
	for x := range distances {
		for y := range distances[x] {
			if board[x][y] == clearSpace && distances[x][y] >= minDoors {
				numRooms++
			}
		}
	}
...
func main() {
	...
	const minRoomDist = 1000
	numFarRooms := findNumFarRooms(board, pos, minRoomDist)
	fmt.Printf("number of rooms farther than %d doors away: %d\n",
		minRoomDist, numFarRooms)
