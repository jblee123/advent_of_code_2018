...
func (a byCartPos) Less(i, j int) bool {
	x1, y1, crashed1 := a[i].pos.x, a[i].pos.y, a[i].crashed
	x2, y2, crashed2 := a[j].pos.x, a[j].pos.y, a[j].crashed
...
func checkForCrash(carts [](*cart)) bool {
	for i := 0; i < len(carts)-1 && !carts[i+1].crashed; i++ {
		if carts[i].pos == carts[i+1].pos {
			carts[i].crashed = true
			carts[i+1].crashed = true
			return true
		}
	}
...
func numCartsRemaining(carts [](*cart)) int {
	numCarts := 0
	for _, theCart := range carts {
		if !theCart.crashed {
			numCarts++
		}
	}
...
func main() {
	...
	for {
		...
		executeStep(theMap, carts)
		if numCartsRemaining(carts) <= 1 {
			break
		}
	}

	fmt.Printf("last cart at (%d,%d) on step %d\n",
		carts[0].pos.x, carts[0].pos.y, stepNum)
	printMap(theMap)
}
